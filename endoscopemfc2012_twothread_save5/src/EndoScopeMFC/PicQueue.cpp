...
//Circular queue
struct _picqueue{ 
	unsigned char picqueueBuf[MAX_BUF_NUM][MAX_BUF_LEN];
	int font, rear;
	int num;
	int flag;
	int len[MAX_BUF_NUM];
...
typedef struct _multi_picqueue
{
	struct _picqueue channelQueue[MAX_CHANNEL_NUM];
	int channel_num;
}multi_picqueue;

#if defined(__WIN32__) || defined(_WIN32)
static HANDLE queueLock[MAX_CHANNEL_NUM]={NULL};
#else
static pthread_mutex_t queueLock;
#endif

static multi_picqueue dataQueue;
...
#if defined(__WIN32__) || defined(_WIN32)
		queueLock[i] = CreateMutex(NULL,FALSE,"pic lock");
		if(queueLock[i]==NULL){
			TRACE("==== %d %d===\n",__LINE__,i);
		}
		Sleep(10);
...
{	
	if(!_have_init_fifo)InitPicQueue();
	int i =0;
	...
	memset(dataQueue.channelQueue[i].picqueueBuf[dataQueue.channelQueue[i].rear],0,MAX_BUF_LEN);
	memcpy(dataQueue.channelQueue[i].picqueueBuf[dataQueue.channelQueue[i].rear],buffer,length);
...
int ReadPicQueue(unsigned char *buffer,int *length)
{
	...
	*length=dataQueue.channelQueue[i].len[dataQueue.channelQueue[i].font];
	memcpy(buffer,dataQueue.channelQueue[i].picqueueBuf[dataQueue.channelQueue[i].font],*length);
